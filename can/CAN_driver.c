...
void vCAN_ReceiveSlotSetting( void )
{
	...
	for( lp_cnt = (unsigned char)CAN_CLR; lp_cnt < CAN_RCV_SLOT_NUM; lp_cnt ++ ) {
		...
		for( lp_wait=(unsigned char)CAN_CLR; ((C0IF1CMDRQ_H&CAN_BIT7)==CAN_BIT7) && (lp_wait<CAN_DATA_TRANS_WAIT); lp_wait++ )
		{
...
void vCAN_SendSlotSetting( void )
{
	...
	for( lp_wait=(unsigned char)CAN_CLR; ((C0IF2CMDRQ_H&CAN_BIT7)==CAN_BIT7) && (lp_wait<CAN_DATA_TRANS_WAIT); lp_wait++ )
	{
		/* Wait for data to be transferred from C0IF1 registers to Message Buffers */
	}
}
...
void vCAN_SendIRQ( void )
{
	...
	if( ((C0INTPND2_H&CAN_BIT7) == CAN_BIT7) && ((C0TXRQ2_H&CAN_BIT7) != CAN_BIT7) && ((C0NEWDAT2_H&CAN_BIT7) != CAN_BIT7) ) {/* Data transmission complete */
